...
#echo "sysbench OLTP mysql, vCPU number: $1, Threads number: $2"
echo "sysbench OLTP mysql, Threads number: $1"
	
	sysbench --test=oltp --oltp-table-size=1000000 --mysql-db=dbtest --mysql-user=root --mysql-password=123 prepare > /dev/null
	#./flush > /dev/null
	...
	/usr/bin/sysbench --test=oltp --oltp-table-size=1000000 --mysql-db=dbtest --mysql-user=root --mysql-password=123 --max-time=60 --oltp-read-only=on --max-requests=0 --num-threads=$1 run
	echo "============> sysbench end===============>"
	sysbench --test=oltp --mysql-db=dbtest --mysql-user=root --mysql-password=123 cleanup > /dev/null
